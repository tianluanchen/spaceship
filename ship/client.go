...
func (c *Client) Ping() (time.Duration, error) {
	...
	if err := checkRespReturnErr(resp, http.StatusOK); err != nil {
		return 0, err
	}
	bs := make([]byte, 4)
...
func (c *Client) Get(concurrency int, remoteFile string, localFile string, hook func(beforeDownload bool, supported bool, length int64, n int)) error {
	...
	fw, err := NewFileWriter(localFile)
	if err != nil {
		return err
	}
	defer fw.Close()
	...
	err = c.fetcher.DownloadWithManual(fileURL, supported, length, &FetcherDownloadOption{
...
func NewClient(opt ClientOption) (*Client, error) {
	c := &Client{}
	if u, err := pkg.FixURL(opt.ServerURL); err != nil {
		return nil, err
	} else {
		c.serverURL = u
	...
	opt.ResponsePreInspector = func(when int, resp *http.Response) error {
		return checkRespReturnErr(resp, http.StatusOK, http.StatusPartialContent)
	}
	if fetcher, err := NewFetcher(opt.FetcherOption); err != nil {
		return nil, err
	} else {
		c.fetcher = fetcher
	}
