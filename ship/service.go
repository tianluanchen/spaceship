...
func (srv *Service) onUpload(absPath, relPath string, w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodPost {
		...
		task := srv.tasks.getTask(relPath)
		if task == nil {
...
func (srv *Service) onList(absPath, relPath string, w http.ResponseWriter, r *http.Request) {
	exist, isDir, err := checkPath(absPath)
	if err != nil {
		writeInternalError(w, err.Error())
		return
...
func (srv *Service) onDelete(absPath, relPath string, w http.ResponseWriter, r *http.Request) {
	exist, isDir, err := checkPath(absPath)
	if err != nil {
		writeInternalError(w, err.Error())
		return
...
func (srv *Service) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	relPath := pkg.CleanPath(q.Get("path"))
	...
	if getDirCount(relPath) > 0 {
		writeBadError(w, "Child directories are not supported")
		return
	}
	if subURLPath == "list" && r.Method == http.MethodGet {
		srv.onList(absPath, relPath, w, r)
		return
	}

	if subURLPath == "delete" && r.Method == http.MethodDelete {
		srv.onDelete(absPath, relPath, w, r)
		return
	}
