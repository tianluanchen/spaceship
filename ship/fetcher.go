...
func (fetcher *Fetcher) DownloadWithManual(url string, supported bool, length int64, option *FetcherDownloadOption) (fatalErr error) {
	if option == nil {
		option = &FetcherDownloadOption{}
	}
	if option.Concurrency <= 0 {
		option.Concurrency = 8
	}
	if option.Handler == nil {
		option.Handler = func(index int, start, end, length int64, r io.Reader, ctx context.Context) error { return nil }
	}
	if option.Retry <= 0 {
		option.Retry = 3
	}
	...
	for count < length || length == -1 {
		...
		go func(index int) {
			...
			for j := 0; j < option.Retry; j++ {
				...
				if err := option.Handler(index, start, end, length, resp.Body, ctx); err != nil {
					once.Do(func() {
						fatalErr = err
					})
					cancel()
